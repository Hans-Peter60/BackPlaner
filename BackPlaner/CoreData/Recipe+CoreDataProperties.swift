...
extension Recipe {
    ...
    /// Stable per-recipe identifier used to build local-notification request ids.
    /// A private recipe has no `firestoreId`, so deriving the id from that alone
    /// made every private recipe share the same empty id — scheduling reminders
    /// for a second recipe then replaced the first one's notifications, because
    /// `UNNotificationRequest`s with an identical identifier overwrite each other.
    public var reminderId: String {
        ...
        // Last resort for legacy rows that never got a UUID.
